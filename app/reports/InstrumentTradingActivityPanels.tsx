...
function FlowWindowCard({ window }: { window: InstrumentInvestorFlowWindow }) {
  ...
  return (
    <div className="overflow-hidden rounded-md border border-[#e5e8eb]">
      <div className="bg-[#f7f8fa] px-3 py-3"><h3 className="text-sm font-black">{label}</h3><p className="mt-0.5 text-xs font-bold text-[#8b95a1]">{window.startDate} ~ {window.endDate}</p></div>
      <div className="divide-y divide-[#e5e8eb]">
        {window.categories.map((category) => (
          <div key={category.category} className="px-3 py-3">
            <div className="flex items-center justify-between gap-3"><p className="text-sm font-black">{formatFlowCategory(category.category)}</p><p className={category.netQuantity >= 0 ? "text-sm font-black text-[#f04452]" : "text-sm font-black text-[#3182f6]"}>{category.netQuantity >= 0 ? "+" : ""}{formatNumber(category.netQuantity)}주</p></div>
            <div className="mt-2 grid grid-cols-2 gap-2 text-xs font-bold text-[#6b7684]"><span>매수 {formatNumber(category.buyQuantity)}주</span><span className="text-right">매도 {formatNumber(category.sellQuantity)}주</span><span>매수/매도 {formatPercentOrDash(category.buySellRatio)}</span><span className="text-right">거래 비중 {formatNumber(category.executionShareRate)}%</span><span className="col-span-2">순현금 {formatLargeWon(category.netCashFlow)}</span></div>
          </div>
        ))}
      </div>
    </div>
...
function ActivityMetric({ detail, label, surface = "muted", value }: { detail: string; label: string; surface?: "muted" | "white"; value: string }) {
  const surfaceClass = surface === "white" ? "bg-white" : "rounded-md bg-[#f7f8fa]";
  return <div className={`${surfaceClass} p-3`}><p className="text-xs font-bold text-[#8b95a1]">{label}</p><p className="mt-1 text-base font-black tabular-nums">{value}</p><p className="mt-1 text-xs font-bold leading-5 text-[#8b95a1]">{detail}</p></div>;
}
