...
export default function ReportsPage() {
  ...
  return (
    <main className="min-h-screen bg-stock-surface-muted text-stock-ink">
      <TradingTopBar active="research" />
      <section className="border-b border-stock-border bg-white">
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <p className="text-xs font-black tracking-[0.12em] text-stock-accent">STOCK RESEARCH</p>
          <h1 className="mt-1 text-2xl font-black">종목 보고서</h1>
          <p className="mt-2 max-w-3xl text-sm font-bold leading-6 text-stock-muted">최신 전체 장마감일을 기준으로 종가·거래활동·수급·보유 구조·기업 이벤트를 같은 시점에서 분석합니다. 실시간 호가와 현재 계좌 운영 상태는 섞지 않습니다.</p>
        </div>
      </section>

      <div className="mx-auto grid max-w-7xl gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[300px_minmax(0,1fr)] lg:px-8">
        <aside className="h-fit rounded-lg border border-stock-border bg-white p-3 lg:sticky lg:top-[92px]">
          <div className="flex items-center justify-between gap-3 px-2 py-2">
            <h2 className="text-sm font-black">보고서 종목</h2>
            <span className="text-xs font-bold text-stock-subtle">{instruments.length}종목</span>
          </div>
          <div className="mt-1 space-y-2">
            {instruments.map((instrument) => {
              const selected = instrument.symbol === selectedSymbol;
              return (
                <button key={instrument.symbol} type="button" onClick={() => setRequestedSymbol(instrument.symbol)} className={selected ? "w-full rounded-md bg-stock-accent-surface p-3 text-left ring-1 ring-stock-accent" : "w-full rounded-md bg-stock-surface-muted p-3 text-left transition hover:bg-stock-surface-strong"}>
                  <div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-black">{instrument.name}</p><p className="mt-0.5 text-xs font-bold text-stock-subtle">{instrument.symbol}</p></div><span className="shrink-0 rounded bg-white px-2 py-1 text-xs font-black text-stock-muted">{instrument.market}</span></div>
                </button>
              );
            })}
            {!instrumentsQuery.isLoading && instruments.length === 0 ? <p className="rounded-md bg-stock-surface-muted px-3 py-5 text-sm font-bold text-stock-subtle">등록된 주문장 종목이 없습니다.</p> : null}
          </div>
        </aside>
        ...
        <div className="min-w-0 space-y-4">
          {report ? (
            <>
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-stock-border bg-white px-4 py-3 text-xs font-bold text-stock-muted">
            ...
            <div className="rounded-lg border border-[#ffd8d2] bg-stock-danger-surface px-5 py-8 text-center"><p className="font-black text-stock-danger-strong">종목 보고서를 불러오지 못했습니다.</p><button type="button" onClick={() => void reportQuery.refetch()} className="mt-4 h-11 rounded-md bg-stock-ink px-4 text-sm font-black text-white">다시 조회</button></div>
          ) : (
            <div className="rounded-lg border border-stock-border bg-white px-5 py-12 text-center text-sm font-bold text-stock-subtle">{instrumentsQuery.isLoading || reportQuery.isLoading ? "종목 보고서를 계산하고 있습니다." : "왼쪽에서 종목을 선택하세요."}</div>
          )}
        </div>
      </div>
    </main>
