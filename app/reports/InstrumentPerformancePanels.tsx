...
export function InstrumentPerformanceReport({ report }: { report: InstrumentMarketReport }) {
  ...
  return (
    <ReportSection eyebrow="PERFORMANCE" title="가격 성과와 위험">
      ...
      <p className="mt-3 text-xs font-bold leading-5 text-[#8b95a1]">
  ...
  return (
    <ReportSection eyebrow="MARKET COMPARISON" title="시장 내 순위와 비교">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(280px,0.85fr)]">
        <div className="overflow-hidden rounded-md border border-[#e5e8eb]">
          <div className="grid grid-cols-[minmax(0,1fr)_90px_120px] bg-[#f7f8fa] px-3 py-2 text-xs font-black text-[#8b95a1]">
        ...
        <div>
          ...
          <div className="mt-2 space-y-2">
            {rankings.similarMarketCapitalizationPeers.map((peer) => (
              <div key={peer.symbol} className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 rounded-md bg-[#f7f8fa] px-3 py-2">
                <div className="min-w-0"><p className="truncate text-sm font-black">{peer.name}</p><p className="text-xs font-bold text-[#8b95a1]">{peer.symbol} · 종가 {formatWon(peer.closePrice)}</p></div>
                <div className="text-right"><p className="text-sm font-black">{formatLargeWon(peer.marketCapitalization)}</p><p className={peer.changeRate >= 0 ? "text-xs font-black text-[#f04452]" : "text-xs font-black text-[#3182f6]"}>{formatSignedPercent(peer.changeRate)}</p></div>
              </div>
            ))}
            {rankings.similarMarketCapitalizationPeers.length === 0 ? <p className="rounded-md bg-[#f7f8fa] px-3 py-4 text-sm font-bold text-[#8b95a1]">비교할 다른 종목이 없습니다.</p> : null}
          </div>
        </div>
      </div>
      <p className="mt-3 text-xs font-bold leading-5 text-[#8b95a1]">모든 순위와 비교 종목 가격은 동일한 보고서 기준일의 전체 장마감 스냅샷이 있는 {rankings.instrumentCount}개 종목만 사용합니다.</p>
    </ReportSection>
  );
}
...
function RankRow({ formatter, label, rank }: { formatter: (value: number) => string; label: string; rank: InstrumentMetricRank }) {
  return (
    <div className="grid grid-cols-[minmax(0,1fr)_90px_120px] border-t border-[#e5e8eb] px-3 py-2 text-sm font-bold">
      <span>{label}</span>
      <span className="text-right tabular-nums">{rank.value == null ? "-" : `${rank.rank}/${rank.total}`}</span>
      <span className="text-right tabular-nums text-[#4e5968]">{rank.value == null ? "산정 불가" : formatter(rank.value)}</span>
    </div>
  );
}
...
function SummaryItem({ label, value }: { label: string; value: string }) {
  return <div className="rounded-md bg-[#f7f8fa] px-3 py-2"><p className="text-xs font-bold text-[#8b95a1]">{label}</p><p className="mt-1 text-sm font-black">{value}</p></div>;
}
