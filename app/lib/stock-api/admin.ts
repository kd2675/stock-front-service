...
export type StockAutoMarketConfigPayload = {
  ...
  primaryRegimeCountWeights?: Partial<AutoMarketRegimeCountWeights>;
...
export type UnderwritingSupplySuspensionPayload = {
  changeReason?: string;
};
...
export function getLiquidityProviderMandates(token: string) {
  return authenticatedGetJson<LiquidityProviderMandate[]>(
    token,
    "/api/stock/v1/markets/liquidity-mandates",
  );
}
...
export function getUnderwritingContracts(token: string) {
  return authenticatedGetJson<UnderwritingContract[]>(
    token,
    "/api/stock/v1/markets/underwriting-contracts",
  );
}
...
export function suspendUnderwritingSupply(
  token: string,
  contractId: number,
...
export function getAdminMarketIndex(token: string, historyDays = 7) {
  return authenticatedGetJson<AdminMarketIndex>(
    token,
    `/api/stock/v1/markets/admin/market-index${toQuery({ historyDays })}`,
  );
}
...
export function getAdminInvestorFlowSummary(token: string) {
  return authenticatedGetJson<AdminInvestorFlowSummary>(token, "/api/stock/v1/markets/admin/investor-flow-summary");
}

export function getAdminInvestorFlowHistory(token: string, days = 7) {
  return authenticatedGetJson<AdminInvestorFlowHistory>(
    token,
    `/api/stock/v1/markets/admin/investor-flow-history${toQuery({ days })}`,
  );
}
...
export function getAutoParticipantV5Operations(token: string) {
  return authenticatedGetJson<AutoParticipantV5Operations>(
    token,
    "/api/stock/v1/markets/auto-market/v5/operations",
  );
}
