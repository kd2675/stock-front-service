  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
      <div className="rounded-lg border border-stock-border bg-white p-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-sm font-bold text-stock-muted">주문할 종목을 먼저 선택하세요.</p>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isAdmin ? (
              <button type="button" onClick={onAdminClick} className="h-11 rounded-md bg-stock-ink px-3 text-sm font-black text-white">
                설정 현황
              </button>
            ) : null}
          </div>
      ...
      {isLoading && !summaries.length ? (
        <div className="mt-5 rounded-lg border border-stock-border bg-white px-5 py-10 text-center">
          <p className="text-base font-black text-stock-ink">종목을 불러오는 중입니다.</p>
          <p className="mt-2 text-sm font-bold text-stock-subtle">주문장 종목, 장 상태, 자동장 설정을 함께 확인하고 있습니다.</p>
        </div>
      ) : summaries.length ? (
          ...
          {summaries.map(({ autoConfig, instrument, marketConfig }) => {
            ...
            return (
              <button
                ...
                className="rounded-lg border border-stock-border bg-white p-4 text-left shadow-[0_1px_2px_rgba(0,0,0,0.03)] transition hover:border-stock-accent hover:shadow-[0_8px_24px_rgba(49,130,246,0.10)] focus:outline-none focus:ring-2 focus:ring-stock-accent"
              >
                <div className="flex min-w-0 items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate text-lg font-black">{instrument.name}</p>
                    <p className="mt-1 text-xs font-bold text-stock-subtle">{instrument.symbol} · {instrument.market}</p>
                  </div>
                  <span className={isOpen ? "shrink-0 rounded-sm bg-stock-accent-surface px-2 py-1 text-xs font-black text-stock-accent" : "shrink-0 rounded-sm bg-stock-danger-surface px-2 py-1 text-xs font-black text-stock-danger-strong"}>
                ...
                <div className="mt-4 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3">
                  <div>
                    <p className="text-xs font-bold text-stock-subtle">현재가</p>
                    <p className="mt-1 text-2xl font-black tabular-nums">{formatWon(instrument.currentPrice)}</p>
                  </div>
                  <p className={changeRate >= 0 ? "text-right text-sm font-black tabular-nums text-stock-danger" : "text-right text-sm font-black tabular-nums text-stock-accent"}>
                    {formatSignedPercent(changeRate)}
                  </p>
                </div>

                <div className="mt-4 grid gap-2 text-xs font-bold text-stock-muted">
                  ...
                  <SelectionInfoRow label="자동장" value={autoConfig?.enabled ? `가동, 최대 ${formatNumber(autoConfig.maxOrderQuantity)}주` : "정지"} />
                </div>
              </button>
            );
          })}
        ...
        <div className="mt-5 rounded-lg border border-stock-border bg-white px-5 py-10 text-center">
          <p className="text-base font-black text-stock-ink">등록된 수요와 공급 종목이 없습니다.</p>
          <p className="mt-2 text-sm font-bold text-stock-subtle">관리자 화면에서 주문장 종목을 만든 뒤 자동장과 거래를 시작할 수 있습니다.</p>
          {isAdmin ? (
            <button type="button" onClick={onAdminClick} className="mt-5 h-11 rounded-md bg-stock-ink px-4 text-sm font-black text-white">
              관리자 설정으로 이동
            </button>
          ) : null}
        </div>
...
function SummaryPill({ label, value, tone = "default" }: { label: string; value: string; tone?: "default" | "blue" }) {
  const valueClass = tone === "blue" ? "text-stock-accent" : "text-stock-ink";
  return (
    <span className="grid h-10 grid-cols-[auto_auto] items-center gap-2 rounded-md bg-stock-surface-strong px-3 text-xs font-bold text-stock-muted">
      {label}
...
function SelectionInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid min-w-0 grid-cols-[76px_minmax(0,1fr)] items-center gap-2 rounded-md bg-stock-surface-muted px-3 py-2">
      <span className="text-stock-subtle">{label}</span>
      <span className="min-w-0 truncate text-right text-stock-text-secondary">{value}</span>
    </div>
  );
}
