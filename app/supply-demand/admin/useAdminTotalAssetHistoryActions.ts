...
import type { AdminParticipantScope, AdminTotalAssetHistoryPage } from "@/app/types/stock";
...
export function useAdminTotalAssetHistoryActions({
  ...
  const loadAdminTotalAssetHistory = async (page: number, participantScope: AdminParticipantScope): Promise<AdminTotalAssetHistoryPage | null> => {
    ...
    try {
      return await unwrapStockRequest(
        getAdminTotalAssetHistory(token, page, participantScope),
