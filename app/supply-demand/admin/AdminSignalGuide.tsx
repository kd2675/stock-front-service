...
export function AutoMarketConfigGuide() {
  const items = [
    ...
    },
    {
      ...
      description: "압력 결과값이 아니라 삼각분포의 최빈 위치입니다. 같은 편향에서도 -100~100 사이의 다른 값이 나올 수 있으며, 주 압력 60%와 보조 압력 40%를 합쳐 최종 신호를 만듭니다.",
    },
    {
      name: "1회 주문 최대 수량",
...
const ORDER_BEHAVIOR_GUIDE_ITEMS = [
  {
    ...
    low: "− 하락·매도 우세",
    high: "+ 상승·매수 우세",
    description: "매수·매도 선택 확률과 주문가격의 위·아래 방향을 움직입니다. 최신 종목 보고서 점수도 참여자 프로필의 뉴스 민감도만큼 이 축에 추가됩니다.",
    caution: "미래 가격을 예측하거나 현재가를 직접 변경하는 값은 아닙니다.",
  },
  {
    name: "자산 선호",
    scale: "-100~100",
  ...
  },
  {
    ...
    low: "− 좁은 호가 범위",
    high: "+ 넓은 호가 범위",
    description: "주문가격을 현재가 주변에 얼마나 넓게 흩을지 조절합니다. 최종 -100은 약 0.35배, +100은 약 1.65배의 가격 분산 배율로 적용됩니다.",
  ...
  },
  {
    ...
    low: "− 수동 지정가",
    high: "+ 상대 호가 교차",
    description: "매수는 최우선 매도호가, 매도는 최우선 매수호가 쪽으로 가격을 내 체결을 시도할 확률을 조절합니다. 양수일수록 즉시 체결 가능성이 커집니다.",
    caution: "+100도 계좌·자기체결·가격 제한을 무시하지 않으며, 체결을 보장하지 않습니다.",
  },
  {
    name: "주문 활동 강도",
    scale: "1~10",
    ...
    caution: "압력처럼 랜덤 생성되지 않습니다. 프로필별 주문·공격성 배율과 노이즈가 이후 단계에서 별도로 적용됩니다.",
  },
] as const;
