    ...
    <details className="group mt-5 rounded-md border border-stock-accent/20 bg-[#10233a]">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        ...
        <div className="rounded-md border border-admin-warning/20 bg-admin-warning/[0.06] px-3 py-3 text-xs font-bold leading-5 text-[#d8c49c]">
          ...
          <p className="mt-1">
            활동 강도 자체는 상승·하락 방향, 1회 주문 수량, 보유 목표, 주문 재검토 기준을 정하지 않습니다. 방향은 다섯 압력·프로필·보고서·수익률·계좌 상태가 정하며, 활동 강도는 자발적 관심 빈도와 호가 가격 반응 크기를 조절합니다.
          </p>
        </div>
      </div>
      </div>
...
export function AutoMarketConfigGuide() {
  const items = [
    ...
    {
      name: "미체결 호가 재검토 기준(초)",
      description: "자동참여자가 미체결 주문을 처음 다시 생각하기까지의 시뮬레이션 시간 기준입니다. 프로필별 배율과 인내심을 거쳐 유지·취소·재가격을 판단하며 시간 도달만으로 일괄 취소하지 않습니다. 장 마감과 운영 안전 사유에 따른 취소는 별도로 유지됩니다.",
    },
  ];
