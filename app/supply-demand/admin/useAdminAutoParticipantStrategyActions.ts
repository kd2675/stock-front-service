...
export function useAdminAutoParticipantStrategyActions({
  ...
  const submitAutoStrategy = async () => {
    ...
    const token = await requireAdminToken("관리자 로그인 후 참여자 종목별 주문 활동 강도를 저장할 수 있습니다.");
    if (!token) {
      return;
    }
    ...
    if (reportAdminActionFailure(result, "참여자 종목별 주문 활동 강도 저장에 실패했습니다.", setMessage)) {
      return;
    }
    setMessage("참여자 종목별 주문 활동 강도를 저장했습니다.");
  ...
  const toggleAutoStrategyEnabled = async (config: AutoParticipantSymbolConfig) => {
    ...
    const token = await requireAdminToken("관리자 로그인 후 참여자 종목별 주문 활동 강도 상태를 변경할 수 있습니다.");
    if (!token) {
      return;
    }
    ...
    if (reportAdminActionFailure(result, "참여자 종목별 주문 활동 강도 상태 변경에 실패했습니다.", setMessage)) {
      return;
    }
    if (strategyUserKey === config.userKey && strategySymbol === config.symbol) {
      setStrategyEnabled(nextEnabled);
    }
    setMessage(nextEnabled ? "참여자 종목별 주문 활동 강도를 가동했습니다." : "참여자 종목별 주문 활동 강도를 정지했습니다.");
    reloadAutoParticipantStrategyState();
  };
