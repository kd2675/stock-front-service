    ...
    <section className="mt-5 rounded-lg border border-white/10 bg-white/[0.06] p-4">
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <button type="button" onClick={onPublish} disabled={saving} className="min-h-11 rounded-md bg-white px-3 py-3 text-sm font-black text-[#101418] disabled:opacity-50">
            {saving ? "저장 중" : "발행"}
          </button>
          ...
          <button type="button" onClick={onDelete} disabled={deleting} className="min-h-11 rounded-md bg-[#3a1f1b] px-3 py-3 text-sm font-black text-[#ffb4a8] disabled:opacity-50">
            {deleting ? "삭제 중" : "삭제"}
          </button>
        </div>
      </div>
