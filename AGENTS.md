...
- `/admin/flows`, `/admin/flows/auto-participants`, `/admin/flows/institutions`, `/admin/flows/users`, `/admin/flows/others` (독립 `시장 흐름` 업무 영역의 종합·자동참여자·기관·개인·기타 페이지)
...
- 탈퇴 감사 화면은 `WITHDRAWN` 생명주기 범위만 읽고 자동 폴링하지 않습니다. 탈퇴 시 주식 반납·현금 회수·계좌 종료 결과를 표시하고, 남은 보유주식·현금·미체결 주문·매도 예약·활성 예산은 운영 점검 신호로 구분합니다.
...
- 참여자별 체결 이력은 `sourceStatus`를 기준으로 장중·장마감 스냅샷·무거래·대기·실패·누락을 구분하고, 대기·실패·누락 날짜를 정상 0건으로 합산하지 않습니다.
- 시장 흐름은 `시장 운영`에서 분리한 독립 관리자 내비게이션 그룹이며 `종합`(기존 전체 화면), `자동참여자`, `기관`, `개인(유저)`, `기타`의 다섯 URL 페이지로 제공합니다. 기관은 `INSTITUTIONAL_INVESTOR`·`LIQUIDITY_PROVIDER`·`ISSUE_UNDERWRITER`를 합계와 원천 역할로 모두 보여주며, 기타는 현재 `SYSTEM_CUSTODY`를 표시합니다. 역할별 breakdown 응답은 로컬에서 전환해 페이지마다 동일 원장을 재요청하지 않고 누적 흐름과 7일 장마감 이력에도 같은 범위를 전달합니다.
- 시장 흐름의 종목값과 전체 장 지수는 응답의 거래일·`sourceStatus`를 함께 표시합니다. 장마감 후 00시 전에는 당일 마감 고정값을 유지하고, 공식 일별 스냅샷 완료 뒤에만 완료값으로 전환합니다.
