...
- `/admin/market/liquidity-providers` (종목별 권장 LP 수치, LP 단건 SHADOW 생성과 LIVE 전환·일일 한도 감사)
- `/admin/market/legacy-liquidity` (전환 전 레거시 상장주관사 자동호가 설정)
- `/admin/participants/institutions` (권장 기관 개수·AUM·운용 유형 수치, 기관 단건 생성, SHADOW·PILOT 감사)
...
- 시장 흐름의 자산·자금 범위는 `전체`, `유저`, `자동 참여자`, `상장주관사` 탭으로 통일합니다. 한 역할별 breakdown 응답을 로컬에서 전환하고 탭마다 동일한 원장 집계를 재요청하지 않으며, 누적 흐름과 7일 장마감 이력에도 같은 범위를 전달합니다.
