...
- `/portfolio`
- `/research` - 종목 분석
- `/news` - 시뮬레이션 시각에 공개된 공시·뉴스와 정정·철회 이력
- `/admin/corporate/news` - 수동 구조화 공시와 종목별 자동 storyline 패턴·단계·다음 후보시각 감사
- `/corporate-actions`
- `/admin/**` - 관리자 운영 화면
...
- `/admin/corporate/news` - 구조화된 시장 사실을 공개·예약하고 lineage별 정정·철회 이력을 관리

이전 `/supply-demand`, `/reports`, `/supply-demand/admin/**` 주소는 canonical route로 영구 redirect합니다. 사용자 화면은 상단 6개 탭, 관리자 화면은 독립 상단 바와 업무별 좌측 navigation을 사용합니다.
...
- 관리자 시뮬레이션 경계 이동은 `오늘 18:00 장마감`, `다음 일자 00:00 야간 처리`, `04:30 증권·시장 데이터 준비`, `05:30 자동장·개장 검증`, `06:00 다음 장 개장`의 다섯 시간 게이트로 구분합니다. 각 버튼은 EOD Job을 직접 실행하지 않고 시계만 이동하며, 서버가 직전 phase 묶음과 거래일 정합성을 확인해 반환한 `availableJumpActions`만 활성화합니다. 중간 시간대를 건너뛴 18:00→06:00 직행은 허용하지 않으며, 1초 상태 조회는 주문·체결 원장을 읽지 않습니다.
