...
const baseAction = {
  ...
  subscriptionStartDate: "2026-07-13",
  subscriptionEndDate: "2026-07-14",
...
assert.equal(resolveState({ currentDate: "2026-07-15", entitlement: shareholderRight }).label, "청약 마감");
assert.equal(resolveState({ currentDate: "2026-07-13", entitlement: shareholderRight, marketSession: "REGULAR" }).label, "장 마감 후");
assert.equal(resolveState({ currentDate: "2026-07-13", marketSession: "AFTER_CLOSE" }).label, "권리 없음");
assert.equal(resolveState({ currentDate: "2026-07-13", entitlementsReady: false }).label, "권리 확인");
...
const partialShareholder = resolveState({
  currentDate: "2026-07-13",
  entitlement: {
    ...shareholderRight,
    subscribedShareQuantity: 10,
    subscribedCashAmount: 50_000,
    status: "PARTIALLY_SUBSCRIBED",
  },
  marketSession: "AFTER_CLOSE",
});
assert.equal(partialShareholder.kind, "ready");
assert.equal(partialShareholder.maxShares, 15);
assert.equal(partialShareholder.label, "접수 가능");
assert.equal(getMaxSubscribableShares(baseAction, {
  ...shareholderRight,
  subscribedShareQuantity: 10,
  forfeitedShareQuantity: 15,
  status: "SUBSCRIBED",
}), 0);
assert.equal(getMaxSubscribableShares(baseAction, {
  ...shareholderRight,
  forfeitedShareQuantity: 25,
  status: "EXPIRED",
}), 0);
...
assert.equal(isCapitalIncreaseOpen(baseAction, "2026-07-13"), true);
assert.equal(isCapitalIncreaseOpen({ ...baseAction, status: "ANNOUNCED" }, "2026-07-13"), false);
assert.equal(isCapitalIncreaseOpen(publicOffering, "2026-07-13"), true);
